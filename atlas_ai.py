...
from javax.swing import (
    ...
    BoxLayout, Box, SwingUtilities, JFileChooser
)
from java.util import ArrayList
from javax.swing import JMenuItem
from java.io import PrintWriter, InputStreamReader, BufferedReader, File, FileInputStream
...
class AtlasAIExtension(IBurpExtender, ITab, IContextMenuFactory, IMessageEditorTabFactory, IScannerListener):
        ...
        try:
            if backend == "openai":
                ...
                if api_key and model:
                    self._current_adapter = OpenAIAdapter(api_key, model, timeout=config_data.get("timeout", 60))
            elif backend == "gemini":
                api_key = config_data.get("gemini_api_key")
                model = config_data.get("gemini_model")
                vision = config_data.get("gemini_use_vision", False)
                if api_key and model:
                    self._current_adapter = GeminiAdapter(api_key, model, timeout=config_data.get("timeout", 60), vision_enabled=vision)
            elif backend == "mistral":
                api_key = config_data.get("mistral_api_key")
                model = config_data.get("mistral_model")
                if api_key and model:
                    self._current_adapter = MistralAdapter(api_key, model, timeout=config_data.get("timeout", 60))
            elif backend == "groq":
                api_key = config_data.get("groq_api_key")
                model = config_data.get("groq_model")
                if api_key and model:
                    self._current_adapter = GroqAdapter(api_key, model, timeout=config_data.get("timeout", 60))
            elif backend == "local":
            ...
            self._stderr.println("[Atlas AI] Adapter Init Error: " + str(e))
            self._update_status("Configuration Error", Color.RED)
    ...
    def test_connection(self):
        ...
        if not self._current_adapter:
            JOptionPane.showMessageDialog(self._main_panel, "Please configure and save your settings first", "No Configuration", JOptionPane.WARNING_MESSAGE)
            return
    ...
    def analyze_image_from_file(self):
        ...
        if not (isinstance(self._current_adapter, GeminiAdapter) and self._config.get("gemini_use_vision")):
            self._log_to_ui("Image analysis is only available with Gemini and the 'Use Vision' option enabled.")
            return

        file_chooser = JFileChooser()
        file_chooser.setDialogTitle("Select an Image for Analysis")
        return_val = file_chooser.showOpenDialog(self._main_panel)

        if return_val == JFileChooser.APPROVE_OPTION:
            selected_file = file_chooser.getSelectedFile()
            
            prompt = JOptionPane.showInputDialog(self._main_panel, "Enter your analysis prompt (e.g., 'Analyze this screenshot for vulnerabilities'):")
            if not prompt:
                return

            def perform_image_analysis():
                try:
                    self._log_to_ui("Analyzing image: " + selected_file.getName())
                    ...
                    if not mime_type:
                        SwingUtilities.invokeLater(lambda: self._log_to_ui("Error: Unsupported image format."))
                        return

                    fis = FileInputStream(selected_file)
                    image_bytes = bytearray(selected_file.length())
                    fis.read(image_bytes)
                    fis.close()
                    
                    response = self._current_adapter.send_message(prompt, image_bytes=bytes(image_bytes), mime_type=mime_type)
                    
                    result = "VISUAL ANALYSIS RESULT\n" + "=" * 60 + "\n"
                    result += "File: " + selected_file.getName() + "\n"
                    ...
                    self._stderr.println("[Atlas AI] Image analysis error: " + str(e))
                    SwingUtilities.invokeLater(lambda: self._log_to_ui("ERROR during image analysis: " + str(e)))

            thread = threading.Thread(target=perform_image_analysis)
            thread.daemon = True
            thread.start()
